...
<img src="ggplot_course_assets/economist-scatterplot.png">
</div>

<div class="pull-right-50">
<img src="ggplot_course_assets/wsj-heatmap.png">
</div>
...
![](ggplot_course_assets/economist-scatterplot.png)
...
```{r}
ec <- read.csv("ggplot_course_assets/data_economist.csv",header=T)
head(ec)
```
...
![](ggplot_course_assets/wsj-heatmap.png)
...
```{r}
me <- read.csv("ggplot_course_assets/data_wsj.csv",header=T,stringsAsFactors=F,skip=2)
head(me)
```
...
The solution is to sum up all the cases for a state for all weeks within a year into one value for that year. This can be done using the `summarise()` function from package `dplyr`. The dots in state names are replaced by spaces and the words are converted to title case (First letter capital and rest lowercase). We also convert the column names to lowercase for consistency.
...
p <- ggplot(me3,aes(x=year,y=state,fill=total))+
            geom_tile()
p
```
